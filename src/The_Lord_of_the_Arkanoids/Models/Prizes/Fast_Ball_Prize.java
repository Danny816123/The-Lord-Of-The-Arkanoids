...
public class Fast_Ball_Prize extends Prize{

    public Fast_Ball_Prize(int width, int height, int x, int y) {
    ...
    public void usePrize(Player myPlayer) {
        System.out.println("Fast_Ball_Prize");
        this.setUtilized(true);
        Timer timer = new Timer();
        myPlayer.setScore(myPlayer.getScore() + 10);
        Fast_Ball_Prize f = this;
        Player iPlayer = new Player(myPlayer);
        for (int i = 0; i < myPlayer.getBallsList().size(); i++) {
            if (myPlayer.getBallsList().get(i).getxVelocity() > 0) myPlayer.getBallsList().get(i).setxVelocity(4);
            else myPlayer.getBallsList().get(i).setxVelocity(-4);
            if (myPlayer.getBallsList().get(i).getyVelocity() > 0) myPlayer.getBallsList().get(i).setyVelocity(4);
            else myPlayer.getBallsList().get(i).setyVelocity(-4);
        }
